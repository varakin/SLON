...
class OARegistrationForgotPasswordFalse(unittest.TestCase):
    def setUp(self):
    ...
    def test_o_a_registration_forgot_password_false(self):
        driver = self.driver
        ...
        driver.find_element_by_name("contact").send_keys("qwertyuiop")
        driver.find_element_by_css_selector("button.form-button.enter").click()
        try: self.assertEqual(u"Некорректный email или номер телефона.", driver.find_element_by_css_selector("h3").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
