...
class OARegistrationForgotPasswordTrueEmail(unittest.TestCase):
    def setUp(self):
    ...
    def test_o_a_registration_forgot_password_true_email(self):
        driver = self.driver
        ...
        time.sleep(1)
        driver.find_element_by_name("contact").send_keys("varakin.1@mail.ru")
        time.sleep(1)
        driver.find_element_by_css_selector("button.form-button.enter").click()
        time.sleep(1)
        try: self.assertEqual(u"Мы отправили Вам инструкцию по сбросу пароля.", driver.find_element_by_css_selector("h3").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
