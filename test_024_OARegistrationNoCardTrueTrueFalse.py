...
class OARegistrationNoCardTrueTrueFalse(unittest.TestCase):
    def setUp(self):
    ...
    def test_o_a_registration_no_card_true_true_false(self):
        driver = self.driver
        ...
        time.sleep(1)
        try: self.assertEqual(u"Регистрация", driver.find_element_by_css_selector("h4").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertEqual(u"Карта активации поставляется вместе с сигнализациями и GSM-модулями M21, M31 и M32", driver.find_element_by_xpath("//form[@id='check-card-form']/div/div/span[2]").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        ...
        try: self.assertEqual(u"я знаю номер SIM-карты моего устройства", driver.find_element_by_id("haveGsm").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        driver.find_element_by_name("email").clear()
        time.sleep(1)
        driver.find_element_by_name("email").send_keys("varakin.1")
        time.sleep(1)
        ...
        time.sleep(1)
        try: self.assertEqual(u"В качестве логина может быть указан только действующий Email", driver.find_element_by_css_selector("h3").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        ...
        driver.find_element_by_name("passwordConfirmation").clear()
        time.sleep(1)
        driver.find_element_by_name("passwordConfirmation").send_keys("654321")
        time.sleep(1)
        driver.find_element_by_name("LoginForm[rememberMe]").click()
        time.sleep(1)
        driver.find_element_by_css_selector("button.form-button.register").click()
        time.sleep(1)
        try: self.assertEqual(u"Введенные пароли не совпадают.", driver.find_element_by_css_selector("h3").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        driver.find_element_by_css_selector("div.error > div").click()
        time.sleep(1)
        try: self.assertEqual("", driver.find_element_by_name("email").text)
