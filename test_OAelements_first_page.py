...
class OAElementsFirstPage(unittest.TestCase):
    ...
    def test_o_a_elements_first_page(self):
        ...
        try: self.assertEqual(u"Ñïóòíèêîâûé\nîõðàííî-ìîíèòîðèíãîâûé\nñåðâèñ", driver.find_element_by_css_selector("h1").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertEqual(u"Äåìî-âõîä", driver.find_element_by_xpath("//div[@id='demo']/span[2]").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertEqual(u"Âõîä", driver.find_element_by_xpath("//div[@id='login']/span[2]").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertEqual(u"Ðåãèñòðàöèÿ", driver.find_element_by_xpath("//div[@id='register']/span[2]").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertEqual(u"Çàãðóçèòå StarLine íà Âàø ñìàðòôîí:", driver.find_element_by_css_selector("div.table-cell.title > span").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        ...
        try: self.assertEqual(u"© 2015 ÍÏÎ \"ÑòàðËàéí\"", driver.find_element_by_css_selector("#footer-copyright > span").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertEqual(u"êàòàëîã óñòðîéñòâ StarLine", driver.find_element_by_link_text(u"êàòàëîã óñòðîéñòâ StarLine").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertEqual(u"êàê êóïèòü è óñòàíîâèòü", driver.find_element_by_link_text(u"êàê êóïèòü è óñòàíîâèòü").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
